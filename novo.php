...
foreach ($dados as $key => $value) {
    ...
    if ($value['curso'] == 'info') { ### escolhendo uma informação específica
        $dados_especificos[] = $value;
    }
...
foreach ($dados_especificos as $key => $value) {// tem uma variável pra cada pergunta do json
    foreach ($value as $key => $value) {
        /*echo "<pre>";
        print("{$key} => {$value}");
        echo "<pre>";*/
